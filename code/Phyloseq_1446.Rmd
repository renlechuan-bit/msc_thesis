...
```{r}
dist_list <- readRDS("/data/gpfs/projects/punim2504/msc_thesis/data/unifrac_L2_data/dist_list.rds")

out_dir <- "/data/gpfs/projects/punim2504/msc_thesis/data/unifrac_L2_data/dist_matrices"   
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)  # 如果不存在则新建

for (m in names(dist_list)) {
  d <- dist_list[[m]]
  mat <- as.matrix(d)
  out <- file.path(out_dir, paste0("dist_", m, ".tsv"))  # 拼接完整路径
  write.table(mat, file = out, sep = "\t", quote = FALSE, col.names = NA)
}
